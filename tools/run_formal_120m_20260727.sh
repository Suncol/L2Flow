...
readonly backup_capture="${artifacts}/formal-120m-feeder-capture.csv"
...
"${root}/build-acceptance/accept-realtime-pipeline" \
    ...
    --user-name-file "${artifacts}/.accept-user-name-120m.tmp" \
    --sdk-log-prefix /tmp/l2flow-stage-latency-20260727-formal-120m-valid \
    --report-json "${artifacts}/formal-120m-report.json" \
    --samples-csv "${artifacts}/formal-120m-samples.csv" \
    --duration-seconds 7200 \
    --generation-interval-ms 1000 \
    --generation-timeout-ms 10000 \
    --instrument-store-workers 4 \
    --acquire-repetitions 8 \
    --intraday-store-max-records 450000000 \
    ...
    --partial-session \
    >"${artifacts}/formal-120m-main-summary.json" \
    2>"${artifacts}/formal-120m-main-stderr.log" &
main_pid=$!

"${root}/build-probe/mdl-sdk-feeder-probe" \
    --library /home/sunc/L2Flow/MDL/libmdl_api.so \
    --address 127.0.0.1:9112 \
    --sdk-log-prefix /tmp/l2flow-mdl-sdk-feeder-probe-20260727-formal-120m-valid \
    --timeout-seconds 10 \
    --market-timeout-seconds 10 \
    --monitor-seconds 7200 \
    --minimum-market-messages 1 \
    --maximum-captured-records 450000000 \
    --capture-csv "${backup_capture}" \
    >"${artifacts}/formal-120m-feeder-summary.json" \
    2>"${artifacts}/formal-120m-feeder-stderr.log" &
probe_pid=$!
